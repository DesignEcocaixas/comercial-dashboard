...
module.exports = (usuarioLogado, clientes) => {
    ...
    <div class="container-85 mb-5">
        ...
        <header class="d-flex justify-content-between align-items-center bg-white p-3 shadow-sm rounded border mb-4 animate-up">
            <div class="d-flex align-items-center">
                ...
                <div>
                    <h5 class="mb-0 fw-bold text-dark">Meses Anteriores</h5>
                    <span class="text-muted small">Histórico de clientes arquivados</span>
                </div>
            </div>
            <div>
                <a href="/vendedor" class="btn btn-sm btn-outline-secondary fw-bold px-3 shadow-sm"><i class="fa-solid fa-arrow-left me-1"></i> Voltar ao Painel</a>
            </div>
        </header>
                        ...
                        <tbody id="tabelaArquivadosBody">
                            <tr id="linhaVazia" style="display: none;">
                                <td colspan="5" class="text-center text-muted py-5">
                                    ...
                                    Nenhum cliente no histórico corresponde aos filtros.
                                </td>
                            </tr>
        ...
        return `
                                <tr class="cliente-row" data-texto="${(c.nome + ' ' + (c.regiao || '')).toLowerCase()}" data-ano="${ano}" data-mes="${mes}">
                                    ...
                                    <td>
                                        <div class="d-flex flex-column gap-1" style="max-width: 110px;">
                                            ...
                                            ${c.fechou === 'sim' ? (c.pos_venda === 'sim' ? '<span class="badge bg-success-subtle text-success border border-success-subtle w-100 text-start fw-normal" style="font-size: 0.7rem;"><i class="fa-solid fa-headset me-1"></i> Pós: Sim</span>' : '<span class="badge bg-warning-subtle text-warning border border-warning-subtle w-100 text-start fw-normal" style="font-size: 0.7rem;"><i class="fa-solid fa-headset me-1"></i> Pós: Pend.</span>') : ''}
    ...
    ${clientes.map(c => `
        <div class="modal fade" id="modalEditar${c.id}" tabindex="-1">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-0 shadow-lg animate-modal">
                    <form action="/vendedor/arquivados/editar" method="POST">
        ...
        <div class="modal fade" id="modalExcluir${c.id}" tabindex="-1">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-0 shadow animate-modal">
                    <form action="/vendedor/arquivados/excluir" method="POST">
                        <div class="modal-header bg-danger text-white border-0"><h5 class="modal-title fw-bold"><i class="fa-solid fa-trash-can me-2"></i> Excluir Registro Passado</h5></div>
                        <div class="modal-body p-4 text-center">
                            <input type="hidden" name="id" value="${c.id}">
                            <i class="fa-solid fa-circle-exclamation fa-3x text-danger mb-3"></i>
                            <p>Excluir permanentemente o histórico de <strong>${c.nome}</strong>?</p>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        function aplicarFiltros() {
            ...
            linhasAtuais = todasLinhas.filter(linha => {
                ...
                return passaTexto && passaAno && passaMes;
            });
